...
const MASK_U8_REV: [u8; 9] = [0b1111_1111, 0b111_1111, 0b11_1111, 0b1_1111, 0b1111, 0b111, 0b11, 0b1, 0];
const MASK_U8_REV_BIT: [u8; 9] = [0, 0b10000000, 0b11000000, 0b11100000, 0b11110000, 0b11111000, 0b11111100, 0b11111110, 0b11111111];
const CLEAR_U8: [u8; 9] = [0b1111_1111, 0b1111_1110, 0b1111_1100, 0b1111_1000, 0b1111_0000, 0b1110_0000, 0b1100_0000, 0b1000_0000, 0b0000_0000];

macro_rules! fast_copy_code {
    (be, $target: ident, $target_start: ident, $src: ident, $src_start: ident, $src_stop:ident, $($ty:ty),*) => {
...
pub unsafe fn bit_copy(target: *mut u8, mut target_start: usize, src: *const u8, mut src_start: usize, src_stop: usize) {
...
//    println!("{}, {}, {}", target_start, src_start, src_stop);
    while src_start < src_stop {
        fast_copy_code!(be, target, target_start, src, src_start, src_stop, u128, u64, u32, u16, u8);
    }
//    println!("{}, {}, {}", target_start, src_start, src_stop);
...
fn test_bit_copy_slice() {
    ...
    unsafe {
        bit_copy(target.as_mut_ptr(), 0, src.as_ptr(), 28, 32);
    }
...
fn test_bit_copy_slice_be() {
    ...
    let src = [0b1010_1010, 0b1111_0000, 0b0000_1111, 0b1100_1100, 0b0011_0011, 0b1010_1010, 0b1111_0000, 0b0000_1111, 0b1100_1100, 0b0011_0011, 0b1010_1010, 0b1111_0000, 0b0000_1111, 0b1100_1100, 0b0011_0011, 0b1010_1010, 0b1111_0000, 0b0000_1111, 0b1100_1100, 0b0011_0011];
    let loop_count = 20000000u64;
    let start = Instant::now();
    for _ in 0..loop_count {
        unsafe { bit_copy(target.as_mut_ptr(), 4, src.as_ptr(), 0, (8 + 4 + 2 + 1) * 8); }
    }
    let ms = dur_millis(start.elapsed());
    println!("esplase test 1 : {:3}ms, {:3}MB/s", ms, 15 * 1000 * loop_count / ms / (1024 * 1024));
...
//    }
    ...
    let src = [0xff, 0x01, 0xff, 0x02, 0xff, 0x03, 0xff, 0x04, 0xff, 0x05, 0xff, 0x06, 0xff, 0x07, 0xff, 0x08, 0xff, 0xff, ];
    let start = Instant::now();
    for _ in 0..loop_count {
        unsafe { bit_copy(target.as_mut_ptr(), 6, src.as_ptr(), 1, 16 * 8 + 2); }
    }
    let ms = dur_millis(start.elapsed());
    println!("esplase test 2 : {:3}ms, {:3}MB/s", ms, 16 * 1000 * loop_count / ms / (1024 * 1024));
//    println!("\r\ntarget:");
//    for t in target.iter() {
//        print!("{:08b} ", t);
//    }
...
//    }
    ...
    fn dur_millis(dur: Duration) -> u64 {
